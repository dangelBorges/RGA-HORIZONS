      ...
      <Card className="bg-[#0f172a] border-primary/10 overflow-hidden shadow-lg">
        <CardContent className="p-0 relative h-[450px]">
          ...
          <div className="absolute top-0 left-0 right-0 z-[1000] p-4 pointer-events-none bg-gradient-to-b from-[#0f172a] to-transparent">
             <h3 className="text-[#06b6d4] font-bold text-lg drop-shadow-md">Mapa Chile — Producción</h3>
          </div>
                ...
                >
                  <Popup className="custom-popup-dark">
                    <div className="text-slate-900">
                      <strong className="block text-sm mb-1">{plant.plant_name}</strong>
                      <span className="text-xs">Vol: {parseInt(plant.production_volume).toLocaleString()} kg</span>
                      <br/>
                      <span className="text-xs capitalize text-slate-500">{plant.region}</span>
                    </div>
                  </Popup>
                </CircleMarker>
              );
