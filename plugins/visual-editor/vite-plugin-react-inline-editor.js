...
export default function inlineEditPlugin() {
  return {
    ...
    transform(code, id) {
      if (
        !/\.(jsx|tsx)$/.test(id) ||
        !id.startsWith(VITE_PROJECT_ROOT) ||
      ...
      try {
          ...
          enter(path) {
            if (path.isJSXOpeningElement()) {
              ...
              const alreadyHasId = openingNode.attributes.some(
                (attr) =>
                  t.isJSXAttribute(attr) && attr.name.name === "data-edit-id",
              );
              ...
              const isCurrentElementEditable = checkTagNameEditable(
                openingNode,
                EDITABLE_HTML_TAGS,
              );
              ...
              if (
                !shouldBeDisabledDueToChildren &&
                t.isJSXElement(elementNode) &&
                elementNode.children
              ) {
                const hasEditableJsxChild = elementNode.children.some(
                  (child) => {
                    if (t.isJSXElement(child)) {
                      return checkTagNameEditable(
              ...
              while (currentAncestorCandidatePath) {
                const ancestorJsxElementPath =
                  currentAncestorCandidatePath.isJSXElement()
                    ? currentAncestorCandidatePath
                    : currentAncestorCandidatePath.findParent((p) =>
                        p.isJSXElement(),
                      );
                ...
                if (
                  checkTagNameEditable(
                    ancestorJsxElementPath.node.openingElement,
                    EDITABLE_HTML_TAGS,
                  )
                ) {
                  return;
                ...
                currentAncestorCandidatePath =
                  ancestorJsxElementPath.parentPath;
              }
        ...
        if (attributesAdded > 0) {
          const output = generateSourceWithMap(
            babelAst,
            webRelativeFilePath,
            code,
          );
        ...
        req.on("end", async () => {
          ...
          try {
            ...
            if (!editId || typeof newFullText === "undefined") {
              ...
              return res.end(
                JSON.stringify({ error: "Missing editId or newFullText" }),
              );
            }
            ...
            if (!parsedId) {
              ...
              return res.end(
                JSON.stringify({
                  error: "Invalid editId format (filePath:line:column)",
                }),
              );
            }
            ...
            const targetNodePath = findJSXElementAtPosition(
              babelAst,
              line,
              column + 1,
            );
            ...
            if (!targetNodePath) {
              ...
              return res.end(
                JSON.stringify({
                  error: "Target node not found by line/column",
                  editId,
                }),
              );
            }
            ...
            const isImageElement =
              targetOpeningElement.name &&
              targetOpeningElement.name.name === "img";
            ...
            if (!modified) {
              ...
              return res.end(
                JSON.stringify({ error: "Could not apply changes to AST." }),
              );
            }

            const webRelativeFilePath = path
              .relative(VITE_PROJECT_ROOT, absoluteFilePath)
              .split(path.sep)
              .join("/");
            const output = generateSourceWithMap(
              babelAst,
            ...
            res.end(
              JSON.stringify({
                error: "Internal server error during edit application.",
              }),
            );
          }
        });
